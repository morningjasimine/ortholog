...
sub eblast {
	my ($FileIn, $FileOut) = @_;
	my ($i, $type, $query, $letter, $name, $length, $qbegin, $qend, $sbegin, $send, $idnty, $expect,$sq,$qq,$ss,$qs);
	...
	while (<$FileIn>) {
		if (/^BLASTN/) {
			...
			if($i == 1) {
				print STDERR "$query\t$qbegin\t$qend\t$qs\t$qq\n$name\t$sbegin\t$send\t$ss\t$sq\n";
			...
			if($i == 1) {
				...
				$i = 0; $qbegin = 0; $sbegin = 0;$sq="";$qq="";$ss="";$qs="";
			}
			$name = $1;
		} elsif (/Length = (\d+)/) {
			$length = $1;
			...
			$idnty = $1;
		} elsif(/Strand\s=\s*(\S+)\s*\/\s*(\S+)/) {
					if($1 ne "Plus"){$qs="-";}else{$qs="+";}
					if($2 ne "Plus"){$ss="-";}else{$ss="+";}
